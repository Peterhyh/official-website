import { Container, Row, Col } from 'reactstrap';
import FS from '../../app/assets/img/honorFullStack.png';
import BE from '../../app/assets/img/honorBackend.png';
import python from '../../app/assets/img/python.png';
import webDev from '../../app/assets/img/webDevFundamentals.png';
import sql from '../../app/assets/img/sql.png';
import devOps from '../../app/assets/img/devops.png';
import BS from '../../app/assets/img/bootstrap.png';
import react from '../../app/assets/img/react.png';
import RN from '../../app/assets/img/reactNative.png';
import mongo from '../../app/assets/img/mongodb.png';

const Certificates = () => {
    return (
        <Container>
            <Row>
                <Col className='certificates-section d-flex justify-content-center mt-3'>
                    <h1>Certificates</h1>
                </Col>
            </Row>
            <Row className='d-flex justify-content-center'>
                <Col className='d-flex justify-content-center mt-3' md='4'>
                    <img className='honor-certificate' src={FS} alt='Full Stack Honor Certificate' />
                </Col>
                <Col className='d-flex justify-content-center mt-3' md='4'>
                    <img className='honor-certificate' src={BE} alt='Backend Honor Certificate' />
                </Col>
            </Row>
            <Row className='d-flex justify-content-center'>
                <Col className='d-flex justify-content-center p-3' md='4'>
                    <p>Full Stack Honor</p>
                </Col>
                <Col className='d-flex justify-content-center p-3' md='4'>
                    <p>Backend Honor</p>
                </Col>
            </Row>

            <Row className='d-flex justify-content-center'>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={webDev} alt='Web Development Fundamentals Certificate' />
                </Col>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={python} alt='Data Structure and Algorithms with Python Certificate' />
                </Col>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={sql} alt='SQL and Data Modeling with Python Certificate' />
                </Col>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={devOps} alt='Modern Software Engineering with DevOps Certificate' />
                </Col>
            </Row>


            <Row className='d-flex justify-content-center'>
                <Col className='d-flex justify-content-center p-3' md='3'>
                    <p>Web Development Fundamentals</p>
                </Col>
                <Col className='d-flex justify-content-center p-3' md='3'>
                    <p>Data Structure and Algorithms with Python</p>
                </Col>
                <Col className='d-flex justify-content-center p-3' md='3'>
                    <p>SQL and Data Modeling with Python</p>
                </Col>
                <Col className='d-flex justify-content-center p-3' md='3'>
                    <p>Modern Software Engineering with DevOps</p>
                </Col>
            </Row>
            <Row className='d-flex justify-content-center'>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={BS} alt='Web Development Fundamentals Certificate' />
                </Col>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={react} alt='Data Structure and Algorithms with Python Certificate' />
                </Col>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={RN} alt='SQL and Data Modeling with Python Certificate' />
                </Col>
                <Col className='d-flex justify-content-center mt-3' md='3'>
                    <img className='fs-certificate' src={mongo} alt='Modern Software Engineering with DevOps Certificate' />
                </Col>
            </Row>
        </Container>
    )
};

export default Certificates;